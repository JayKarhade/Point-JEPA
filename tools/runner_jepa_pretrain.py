...
def jepa_run_net(args, config, train_writer=None, val_writer=None):
    ...
    # optimizer & scheduler
    optimizer, scheduler = builder.build_opti_sche_jepa(base_model, config)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if num_iter == config.step_per_update:
                ...
                #update student model with momentum update

                m=0.996 #momentum

                student_model = base_model.module.target_encoder.eval()
                teacher_model = base_model.module.context_encoder.eval()

                # with torch.no_grad():
                #     for student_param, teacher_param in zip(student_model.parameters(), teacher_model.parameters()):
                #         print(student_param,teacher_param)
                #         teacher_param.data.mul_(other=m).add_(other=student_param.data, alpha=1 - m)

                with torch.no_grad():
                    # m = next(momentum_scheduler)
                    for param_q, param_k in zip(teacher_model.parameters(), student_model.parameters()):
