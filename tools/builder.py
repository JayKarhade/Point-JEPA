...
def build_opti_sche_jepa(base_model, config):
    opti_config = config.optimizer
    if opti_config.type == 'AdamW':
        def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
            decay = []
            no_decay = []
            for name, param in model.module.named_parameters():
                if name.split('.')[0]=='target_encoder': # we dont want to backprop to target encoder
                    # print("Not adding target encoder")
                    continue
                # print(name.split('.')[0])
                if not param.requires_grad:
                    continue  # frozen weights
        ...
        #only update teacher and predictor weights
        # print((base_model.__dir__))
        # teacher_model = base_model.__dict__['_modules']['context_encoder']
        # print(teacher_model.__dict__['_modules'].keys())
        # predictor = base_model.__dict__['_modules']['predictor']

        # param_groups = add_weight_decay(teacher_model, weight_decay=opti_config.kwargs.weight_decay)
        # param_groups = add_weight_decay(predictor, weight_decay=opti_config.kwargs.weight_decay)
